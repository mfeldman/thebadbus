...
class KDTree(object):
    ...
    """
    def __init__(self, data, leafsize=10, names = np.array([])):
        ...
        if len(names) == 0:
            names = np.array(range(len(data)))
        self.names = names
        self.tree = self.__build(np.arange(self.n), self.maxes, self.mins, self.names)
    ...
    def __build(self, idx, maxes, mins, names):
        if len(idx)<=self.leafsize:
            ...
            return KDTree.innernode(d, split,
                    self.__build(idx[less_idx],lessmaxes,mins, names[less_idx]),
                    self.__build(idx[greater_idx],maxes,greatermins, names[greater_idx]))
